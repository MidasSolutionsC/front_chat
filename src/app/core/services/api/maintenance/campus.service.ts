...
import { CampusList, ResponseApi } from 'src/app/core/models';
...
export class CampusService {
  ...
  private listSubject: BehaviorSubject<CampusList[]> = new BehaviorSubject<CampusList[]>([]);
  public listObserver$: Observable<CampusList[]> = this.listSubject.asObservable();
  ...
  private get baseUrl(){
    return this.configService.apiUrl + 'campus';
  }
  ...
  addObjectObserver(campusList: CampusList) {
    const currentData = this.listSubject.getValue();
    currentData.push(campusList);
    this.listSubject.next(currentData);
  }

  // Método para actualizar todo el array
  addArrayObserver(campusList: CampusList[]) {
    this.listSubject.next(campusList);
  }
